...
import { meilisearchErrorCodes, MeilisearchModuleOptions } from "./types";
...
export class MeiliSearchService extends SearchUtils.AbstractSearchService {
    ...
    protected readonly config_: MeilisearchModuleOptions;
    protected readonly client_: MeiliSearch;

    constructor(container: any, options: MeilisearchModuleOptions) {
        ...
        if (process.env.NODE_ENV !== "development") {
            if (!options.config?.apiKey) {
                throw Error("Meilisearch API key is missing in module configuration.");
            }
        }

        if (!options.config?.host) {
            throw Error("Meilisearch host is missing in module configuration.");
        }
